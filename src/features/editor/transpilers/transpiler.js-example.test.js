...
ClassifyEmailWorkflow(
  c8 => {
    ...
    const { email_content, subject_line, sender_address } =
      c8.extract.extract_email_body_subject_sender(raw_email);
  ...
  c8 => {
    ...
    const { spam_score } = c8.understand.analyze_spam_patterns(
      email_content,
      subject_line,
      sender_address,
    );
  ...
  c8 => {
    ...
    const { is_spam } = c8.logic.classify_email_based_on_spam_score(
      spam_score,
...
class ExtractService extends CoreBlueprint {
   ...
   * @returns {Object} An object containing email content, subject line, and sender address
...
class UnderstandService extends CoreBlueprint {
   ...
   * @returns {Object} An object containing the spam score
...
class LogicService extends CoreBlueprint {
   ...
   * @returns {Object} An object containing the spam classification result
